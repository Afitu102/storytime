...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const stories =
        document.querySelectorAll(".story-card[data-new='true']");
    ...
    stories.forEach(card => {
        ...
        // Only show stories less than 7 days old
        if (days <= 7) {
            ...
            item.innerHTML = `
                <h3>${card.querySelector("h2").textContent}</h3>
                <p>${card.dataset.category}</p>
